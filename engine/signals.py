...
def empirical_zscore(value: float, history: Sequence[float], min_samples: int = 24) -> float:
    ...
    if len(history_values) < min_samples:
        raise ValueError(
            f"Insufficient factor history: {len(history_values)} < {min_samples}"
        )
...
class QuantSignalEngine:
    ...
    def __init__(
        ...
        self.score_tanh_scale = score_tanh_scale
        self.sweep_booster_points = sweep_booster_points
        self.strong_signal_threshold = strong_signal_threshold
        self.swing_buffer_bps = swing_buffer_bps
        ...
        if abs((funding_weight + basis_weight) - 1.0) > 1e-9:
        ...
        if score_tanh_scale <= 0.0 or target_risk_reward <= 0.0:
    ...
    def compute_signal(
        ...
        if not all(x is not None and math.isfinite(float(x)) for x in values):
            raise ValueError("Signal input contains non-finite values")
        ...
        if z_cvd_override is None or z_fund_override is None or z_delta_oi_override is None or z_micro_override is None or z_whale_override is None:
