    ...
    async def fetch_sentiment_divergence(
        ...
        try:
            async with session.get(retail_url, params=params) as resp_r, \
                ...
                timestamps = (retail_ts, whale_ts, taker_ts)
                if len(set(timestamps)) != 1:
                    logger.error("sentiment_timestamp_mismatch symbol=%s timestamps=%s", symbol, timestamps)
                    return None
                if start_time_ms is not None and not all(start_time_ms <= ts <= end_time_ms for ts in timestamps):
                    logger.error("sentiment_timestamp_outside_window symbol=%s timestamps=%s", symbol, timestamps)
                    return None
